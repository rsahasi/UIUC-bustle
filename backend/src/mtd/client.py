...
class _TTLCache:
    """Simple in-memory TTL cache. One TTL per key."""
...
def _normalize_departures_response(stop_id: str, raw: dict[str, Any]) -> dict[str, Any]:
    ...
    dep_list = (
        raw.get("departures")
        or (rsp.get("departures") if isinstance(rsp, dict) else None)
        or (rsp.get("Departures") if isinstance(rsp, dict) else None)
        or []
    )
...
class MTDClient:
    """Async client for Champaign-Urbana MTD Developer API with TTL caching."""
    ...
    async def get_departures_by_stop(self, stop_id: str, minutes: int = 60) -> dict[str, Any]:
        """
        ...
        if cached is not None:
            logger.info("telemetry departures_served cache_hit=true stop_id=%s", stop_id)
            return cached

        logger.info("telemetry departures_served cache_hit=false stop_id=%s", stop_id)
        ...
        for attempt in range(MTD_RETRY_ATTEMPTS):
            try:
                async with httpx.AsyncClient(timeout=MTD_REQUEST_TIMEOUT_SECONDS) as client:
    ...
    async def get_vehicles_in_service(self, route_id: str | None = None) -> list[dict[str, Any]]:
        """
        Get vehicles currently in service. Cached for 10s.
        Returns list of { vehicle_id, lat, lng, heading, route_id, headsign }.
