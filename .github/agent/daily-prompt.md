...
7. MERGE only if the backend test suite (and mobile checks, when changed) passed in
   step 5: `gh pr merge --merge --delete-branch` (merge commit, NOT squash, so each
   logical commit is preserved). If anything failed or is uncertain, leave the PR open
   and say why. Never force-push or rewrite `main`.
8. Keep `docs/PRODUCTION_READINESS.md` current. Print a summary of what you did.

## Constraints
- Real, defensible improvements only; tests must pass before any merge.
- Preserve individual commits on merge (merge commit, not squash).
- One PR per run.
